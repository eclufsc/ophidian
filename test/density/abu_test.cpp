...
TEST_CASE("density/ abu of superblue18","[density][abu]") {
    ...
    openeda::placement::lef::read(lef, &std_cells, &lib, &floorplan);
    openeda::placement::def::read(def, &netlist, &cells, &floorplan);
    ...
    openeda::density::abu abu(&floorplan, &cells, {number_of_rows_in_each_bin * row_height, number_of_rows_in_each_bin * row_height});
