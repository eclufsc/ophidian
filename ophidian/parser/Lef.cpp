...
namespace ophidian
{
    namespace parser
    {
        ...
        void LefParser::readFile(
            const std::string & filename,
            std::unique_ptr <ophidian::parser::Lef> & inp)
        {
            auto fp =
...
//	std::unique_ptr<Lef> inp = std::make_unique<Lef>();
            ...
            lefrSetUnitsCbk(
                ...
                static_cast <Lef *>(ud)->mThis->units_.setDatabase(
                    units->databaseName(),
                ...
                for(int i = 0; i < l->numPorts(); ++i)
                {
                    ...
                    for(int j = 0; j < l->port(i)->numItems(); ++j)
                    {
                        ...
                        case lefiGeomRectE:
                            ...
                            r.firstPoint =
                                util::LocationMicron(
                                    l->port(i)->getRect(j)->xl,
                                    l->port(i)->getRect(j)->yl);
                            r.secondPoint =
                                util::LocationMicron(
                                    l->port(i)->getRect(j)->xh,
                                    l->port(i)->getRect(j)->yh);
            ...
            lefrSetObstructionCbk(
                ...
                for(int i = 0; i < geometries->numItems(); ++i)
                {
                    ...
                    case lefiGeomRectE:
                        ...
                        Lef::rect r {util::LocationMicron(
                                         geom_rect->xl,
                                         geom_rect->yl),
                                     util::LocationMicron(geom_rect->xh, geom_rect->yh)};
