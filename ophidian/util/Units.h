...
//!Creating a new boost point to allow receiving double in the constructor and converting to micrometer_t
                ...
                public:
                    ...
                    inline point_xy_location(double const & x, double const & y):
                            model::point <CoordinateType, 2, CoordinateSystem>(CoordinateType(x),
                                CoordinateType(y))
                    {
                    }
                    ...
                    {
                        return ophidian::geometry::Point(
                            units::unit_cast <double>(this->x()),
                            units::unit_cast <double>(this->y()));
                    }
                };
            }     //namespace d2
        }     //namespace model
...
//! boost bidimensional point for the unit micrometer_t
        using LocationMicron =
            boost::geometry::model::d2::point_xy_location <ophidian::util::micrometer_t>;

//! boost bidimensional point for the unit dbumeter_t
        using LocationDbu =
            boost::geometry::model::d2::point_xy_location <ophidian::util::dbumeter_t>;
...
//!Class to handle the conversion from micron <-> DBU. It has to be instantiated by a def/lef library, which defines the DBU to micron factor
        ...
        public:
            ...
            explicit DbuConverter(int dbu_factor):
            ...
            {
                double conv_value = units::unit_cast <double>(value) /
                                    static_cast <double>(m_dbu_factor);
            ...
            {
                int conv_value_x = units::unit_cast <double>(value.x()) /
                                   static_cast <double>(m_dbu_factor);
                int conv_value_y = units::unit_cast <double>(value.y()) /
                                   static_cast <double>(m_dbu_factor);
