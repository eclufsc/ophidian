...
lef_parser::lef_parser(const std::string &filename)
{
    lefrInit();
    ...
    lefrSetUnitsCbk([](lefrCallbackType_e,
                    lefiUnits* units,
                    lefiUserData ud)->int{
        static_cast<lef_parser*>(ud)->m_units = *units;
        static_cast<lef_parser*>(ud)->m_units.setDatabase(units->databaseName(), units->databaseNumber());
        return 0;
    });
    ...
    lefrSetSiteCbk([](lefrCallbackType_e,
        ...
        s.x = l->sizeX();
        s.y = l->sizeY();
        static_cast<lef_parser*>(ud)->m_sites.push_back(s);
        return 0;
    });
    ...
    lefrSetLayerCbk([](lefrCallbackType_e,
        ...
        lay.pitch = l->pitch();
        lay.width = l->width();
        static_cast<lef_parser*>(ud)->m_layers.push_back(lay);
        return 0;
    });
    lefrSetPinCbk([](lefrCallbackType_e,
                  lefiPin* l,
                  lefiUserData ud)->int{
        macro & m = static_cast<lef_parser*>(ud)->m_macros.back();
        pin p;
        p.name = l->name();
        if(l->hasDirection())
        {
    ...
    lefrSetMacroBeginCbk([](lefrCallbackType_e,
                         const char *string,
                         lefiUserData ud)->int{
        static_cast<lef_parser*>(ud)->m_macros.push_back(macro{string});
        return 0;
    });
...
lef_parser::~lef_parser()
{

}

void lef_parser::site::setXsymmetry()
{
    symmetry |= X;
}

void lef_parser::site::setYsymmetry()
{
    symmetry |= Y;
}

void lef_parser::site::set90symmetry()
{
    symmetry |= NINETY;
}
