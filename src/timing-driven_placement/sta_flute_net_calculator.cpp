...
#pragma omp single
    {
        for (std::unordered_set<entity::entity>::const_iterator net_it = m_dirty.begin(); net_it != m_dirty.end(); ++net_it) {
            auto net = (*net_it);
#pragma omp task firstprivate(net)
            if(m_placement.netlist().net_pins(net).size() != 1)
            {
