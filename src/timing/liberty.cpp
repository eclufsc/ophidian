...
void read_LUT(si2drGroupIdT group,
	...
	loadValues.resize(0); slewValues.resize(0);
	...
	while (!si2drObjectIsNull((attr = si2drIterNextAttr(attrs, &err)), &err)) {
		...
		if (name == "index_1" || name == "index_2" || name == "values") {
			...
			std::size_t i = 0;
			do {
				std::vector<double> values_vector = split_string_into_values(
...
void read_LUTs(si2drGroupIdT timing, entity::entity arc, library& library,
		boost::units::quantity<boost::units::si::time> time_unit,
		boost::units::quantity<boost::units::si::capacitance> capacitive_load_unit) {
...
void read_timing(si2drGroupIdT timing, entity::entity pin_entity,
		library& library,
		boost::units::quantity<boost::units::si::time> time_unit,
		boost::units::quantity<boost::units::si::capacitance> capacitive_load_unit) {
	...
	while (!si2drObjectIsNull((attr = si2drIterNextAttr(attrs, &err)), &err)) {
		...
		if (attr_name == "timing_sense")
			...
			from = library.std_cells().pin_create(
					library.std_cells().pin_owner(pin_entity), related_pin);
		}
	}
	si2drIterQuit(attrs, &err);
...
void read_pin(entity::entity cell_entity, si2drGroupIdT pin, library& library,
	...
	while (!si2drObjectIsNull((attr = si2drIterNextAttr(attrs, &err)), &err)) {
		...
		if (attr_name == "capacitance")
			library.pin_capacitance(pin_entity,
	...
	while (!si2drObjectIsNull((group = si2drIterNextGroup(groups, &err)), &err)) {
		...
		if (group_type == "timing") {
			...
			read_timing(group, pin_entity, library, time_unit,
					capacitive_load_unit);
...
void read_cell(si2drGroupIdT cell, timing::library& library,
	...
	while (!si2drObjectIsNull(
		...
		if (current_cell_group_type == "pin") {
			read_pin(cell_entity, current_cell_group, library, time_unit,
					capacitive_load_unit);
		}
	}
	...
	while (!si2drObjectIsNull((group = si2drIterNextGroup(groups, &err)), &err)) {
		...
		while (!si2drObjectIsNull(
			...
			if (std::string { current_group_type } == "cell")
				parser::read_cell(current_group, library, time_unit,
						capacitive_load_unit);
		}
