...
elmore::elmore(rc_tree& tree, rc_tree::capacitor_id source) :
				m_tree(tree), m_elmore_delay(tree.graph()), m_downstream_capacitance(
						tree.graph()), m_pred(tree.graph()), m_source(source) {
	...
	while (!to_process.empty()) {
		...
		for (rc_tree::resistor_it it { tree.capacitor_resistors(element) };
			...
			auto other = tree.other_capacitor(
					static_cast<rc_tree::resistor_id>(it), element);
			if (reached.insert(other).second) {
				m_pred[other] = std::make_pair(element,
						static_cast<rc_tree::resistor_id>(it));
...
void elmore::update() {
	...
	for (std::vector<rc_tree::capacitor_id>::reverse_iterator it =
			m_order.rbegin(); it != m_order.rend(); ++it) {
		if (m_pred[*it].first != lemon::INVALID)
			m_downstream_capacitance[m_pred[*it].first] +=
					m_downstream_capacitance[*it];
	}
	...
	for (auto c : m_order) {
		if (m_pred[c].first != lemon::INVALID)
			m_elmore_delay[c] = m_elmore_delay[m_pred[c].first]
											   + m_tree.resistance(m_pred[c].second)
											   * m_downstream_capacitance[c];
	}
}
