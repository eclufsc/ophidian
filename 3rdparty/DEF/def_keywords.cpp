    ...
    if (ch == '"') {
       do {
          ...
          */
          ...
          /* 5/5/2004 - pcr 704784
          ** if name, or quote string is longer than DEFSIZE, truncate it
...
int sublex()
{
   char uc_token[DEFSIZE];    /* upper case copy of the token */
   char fc;
   double numVal;
   char*  outMsg;
   ...
   for(;;) {
      ...
      if (fc == defSettings->CommentChar) {
         ...
         /* store them.  Otherwise it's a define, or a macro use. */
          string alias;
         uc_array(defData->deftoken, uc_token);
   ...
   if (defData->dumb_mode >= 0) {
      ...
      if (defData->orient_is_keyword) {
         int result;
         char *ch, *uch;
         for (ch = defData->deftoken, uch = uc_token; *ch != '\0'; ch++)
             *uch++ = toupper(*ch);
             *uch = '\0';
